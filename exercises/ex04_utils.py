...
def all(list: list[int], int: int) -> bool:
    ...
    list_index: int = 0
    number_of_index_matches = 0   # used to count the how many elements in list same as the argument int
    while list_index < len(list):
        if int == list[list_index]:
            number_of_index_matches += 1  # If int exist once in list, add 1
        list_index += 1
    if number_of_index_matches == len(list):  # Return True if all numbers in list match the indicated number int
        return True
    else:
...
def max(input: list[int]) -> int:
    ...
    biggest_int_in_list: int = 0 # Set initial biggest int
    index: int = 0
    while index < len(input):
        if input[index] > biggest_int_in_list: # Change biggest int if we countered a bigger one
            biggest_int_in_list: int = input[index]
        index += 1
    return biggest_int_in_list # After the loop, after we campared all int in list, we save the biggest int
                                # and Return it
